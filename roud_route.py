...
def get_road_route(origin: str, destination: str, car_status: bool, avoid: str = "",
    ...
    if resp.get('status') == 'OK':
        ...
        journey_info = {
            "journey_origin": route['start_address'].split(',')[0:-2] if len(route['start_address'].split(',')) > 2 else
            route['start_address'].split(',')[0],
            "journey_destination": route['end_address'].split(',')[0:-2] if len(
                route['end_address'].split(',')) > 2 else route['end_address'].split(',')[0],
